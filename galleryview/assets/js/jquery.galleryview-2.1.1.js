...
(function($){
	$.fn.galleryView = function(options) {
		...
		var clicked = false;	// BOOLEAN - flag to indicate whether something has been clicked
...
*/
		...
		function showItem(i) {
			...
			if(opts.show_panels && opts.fade_panels) {
				j_panels.fadeOut(opts.transition_speed).eq(i%item_count).fadeIn(opts.transition_speed,function(){
					...
					if(!opts.show_filmstrip) {
						$('.nav-prev-overlay',j_gallery).click(clickShowPrevItem);
						$('.nav-next-overlay',j_gallery).click(clickShowNextItem);
						$('.nav-prev',j_gallery).click(clickShowPrevItem);
						$('.nav-next',j_gallery).click(clickShowNextItem);		
					}
				});
			}
	...
	*/
		...
		function clickShowNextItem() {
			if (opts.pause_on_click) {
				clicked = true;
				paused = true;
			}
			...
			if(opts.pause_on_hover || (opts.show_panels && !opts.show_filmstrip)) {
				$(document).mousemove(function(e){
					if(mouseIsOverGallery(e.pageX,e.pageY)) {
						...
						// && (!opts.pause_on_click || (opts.pause_on_click && clicked == true))
						if( 
							(opts.pause_on_hover && !opts.pause_on_click)
							||
							(opts.pause_on_hover && opts.pause_on_click && clicked == false)
						 ) {
...
*/
	$.fn.galleryView.defaults = {
		...
		overlay_height: 80,					//FLOAT - transparency for panel overlay (1.0 = opaque, 0.0 = transparent)
		...
		pause_on_click: true				//BOOLEAN - flag to pause slideshow when user clicks anything
	};
})(jQuery);