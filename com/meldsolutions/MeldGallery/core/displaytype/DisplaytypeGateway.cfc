...
<!---^^GENERATEDSTART^^--->
	...
	<cffunction name="getByAttributesQuery" access="public" output="false" returntype="query">
		...
		<cfquery name="qList" datasource="#variables.dsn#" username="#variables.dsnusername#" password="#variables.dsnpassword#">
			SELECT
				*,'true' AS BeanExists
			FROM	#variables.dsnprefix#mg_displaytype
			WHERE	0=0
		<!---^^VALUES-START^^--->
	...
	<cffunction name="getByArray" access="public" output="false" returntype="Struct" >
		...
		<cfquery name="qList" datasource="#variables.dsn#" username="#variables.dsnusername#" password="#variables.dsnpassword#">
			SELECT
				*
			FROM
				#variables.dsnprefix#mg_displaytype
			WHERE
		...
		<cfif not arguments.isCount AND variables.dsntype eq "mssql" AND arguments.start gt 0>
			...
			<cfquery name="qKeep" dbtype="query" maxrows="#arguments.count#">  
				...
				FROM  
					qList  
				WHERE  
					displaytypeID NOT IN (<cfqueryparam value="#valueList(qExclude.displaytypeID)#" list="true" />)  
			</cfquery> 
