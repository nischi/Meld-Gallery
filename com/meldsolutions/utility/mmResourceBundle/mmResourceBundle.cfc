...
<cfcomponent displayname="mmResourceBundle" hint="Creates and manages resource bundles" output="false">
	...
	<cffunction name="resetRBFactory" access="public" returntype="void" output="false">
		...
		<cfset rbFactory = createObject("component","mura.resourceBundle.resourceBundleFactory").init(parent,getRBPath(),getBaseRBLocale() ) />
	...
	<cffunction name="doValidateBaseResourceBundle" access="private" returntype="void" output="false">
		...
		<cfset var writefile	= path & "/" & rereplace(getBaseRBLocale(),"\_.*","") & ".properties" />
		...
		<cfif not fileExists( file ) and not fileExists( writefile )>
			<cffile action="write" file="#writefile#" output="## Created by mmResourceBundle #lsDateFormat(now(),"short")#">
		</cfif>

		<cfset variables.instance.rbPath	= path />
		<cfset variables.instance.rbFile	= writefile />
