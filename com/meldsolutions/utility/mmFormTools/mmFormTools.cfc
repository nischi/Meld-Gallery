...
<cfcomponent displayname="mmFormTools" hint="assists Form / Bean interaction" output="false">
	...
	<cffunction name="paramaterizeStruct" access="public" output="false" returntype="void">
		<cfargument name="sourceStruct" required="true" type="struct">
		<cfargument name="prefix" required="true" type="string">

		<cfset var iiX	= "" />

		<cfloop collection="#arguments.sourceStruct#" item="iiX">
			<cfparam name="form['#lcase("#prefix#_#iiX#")#']" default="#arguments.sourceStruct[iiX]#">
		</cfloop>
	</cffunction>
	...
	<cffunction name="paramaterizeBeanBooleans" access="public" output="false" returntype="void">
		...
		<cfloop from="1" to="#arrayLen(aParams)#" index="iiX">
			<cfif aParams[iiX].type eq "boolean">
				...
				<cfif StructKeyExists(sPresets,aParams[iiX].name)>
					<cfset cValue = sPresets[aParams[iiX].name]>
				<cfelseif not StructKeyExists(form,aParams[iiX].name)>
					<cfset cValue = "0">
				<cfelse>
					<cfset cValue = form[aParams[iiX].name]>
				</cfif>
